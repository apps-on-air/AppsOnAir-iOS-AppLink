import Foundation
    ...
    internal class AppLinkApiService {
        ...
        internal static func apiGenerateShortLink(
            ...
            let generateShortLink = self.getURLFromString(
                urlString: EnvironmentConfig.createShortLink)
            ...
            let httpBody = try? JSONSerialization.data(
                withJSONObject: apiShortLinkPassData, options: [])
        ...
        @objc internal static func apiReferralInfo(
            appLinParams: [String: Any] = [:], isEnableAdvancedDeferredLink: Bool = false,
            completion: @escaping ([String: Any]) -> Void
        ) {
            ...
            appHelper.getUserAgent { linkUserAgent in
                ...
                if isEnableAdvancedDeferredLink {
                    request.httpMethod = "POST"
                    let httpBody = try? JSONSerialization.data(
                        withJSONObject: appLinParams, options: [])
                    request.httpBody = httpBody
                }
        ...
        @objc internal static func apiLinkAnalytics(
            isClicked: Bool = true, urlPrefix: String,
            shortId: String? = nil, isInstalled: Bool = false, isFirstOpen: Bool = false,
            completion: @escaping ([String: Any]) -> Void
        ) {
        ...
        @objc internal static func apiFetchLinkInfo(
            domain: String, linkId: String, completion: @escaping ([String: Any]) -> Void
        ) {
            ...
            let linkURL: URL = self.getURLFromString(
                urlString: EnvironmentConfig.getLinkInfo + linkId)
