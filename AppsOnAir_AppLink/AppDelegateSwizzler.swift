    ...
    internal class AppSwizzler {
        ...
        private init() {
            ...
            guard getSceneDelegateClass() != nil else {
        ...
        private func getSceneDelegateClass() -> NSObject.Type? {
            ...
            Logger.logInternal(
                "❌ Swizzling failed: Could not find SceneDelegate class. Tried: \(classNameCandidates)"
            )
            return nil
        }
        ...
        private func swizzleAppDelegateIfNeeded() {
            ...
            let originalSelector = NSSelectorFromString(
                "application:continueUserActivity:restorationHandler:")
            let swizzledSelector = #selector(
                AppSwizzler.swizzled_application(_:continueUserActivity:restorationHandler:))

            guard let swizzledMethod = class_getInstanceMethod(AppSwizzler.self, swizzledSelector)
            else {
                Logger.logInternal(
                    "❌ Swizzled method application:continueUserActivity:restorationHandler not found"
                )
                return
            }
            ...
            if let originalMethod = class_getInstanceMethod(delegateClass, originalSelector) {
                ...
                Logger.logInternal(
                    "Background method added: \(added ? "✅ Added and swizzled method" : "❌ Failed to add method")"
                )
            }
        }
        ...
        private func swizzleAppDelegateForOpenURLIfNeeded() {
            ...
            if let originalMethod = class_getInstanceMethod(delegateClass, originalSelector) {
                ...
                let added = class_addMethod(delegateClass, originalSelector, imp, types)
                Logger.logInternal(
                    "Deep Liking method added: \(added ? "✅ Added and swizzled method" : "❌ Failed to add method")"
                )
            }
        }
        ...
        private func swizzleSceneWillConnectTo(_ sceneDelegateClass: NSObject.Type) {
            ...
            let swizzledSelector = #selector(
                sceneDelegateClass.swizzled_scene(_:willConnectTo:options:))

            guard
                let originalMethod = class_getInstanceMethod(sceneDelegateClass, originalSelector),
                let swizzledMethod = class_getInstanceMethod(sceneDelegateClass, swizzledSelector)
            else {
                Logger.logInternal(
                    "Swizzling failed: Could not find methods to swizzle scene(_:willConnectTo:options:)."
                )
                return
            }
        ...
        private func swizzleSceneContinue(_ sceneDelegateClass: NSObject.Type) {
            ...
            guard
                ...
                Logger.logInternal(
                    "Swizzling failed: Could not find methods to swizzle scene(_:continue:).")
                return
            }
        ...
        private func swizzleSceneOpenURLContexts(_ sceneDelegateClass: NSObject.Type) {
            ...
            let swizzledSelector = #selector(
                sceneDelegateClass.swizzle_scene_continue(_:openURLContexts:))
            ...
            guard
                ...
                Logger.logInternal(
                    "Swizzling failed: Could not find methods to swizzle scene(_:openURLContexts:)."
                )
                return
            }
    ...
    extension AppSwizzler {
        ...
        @objc func swizzled_application(
            ...
            Logger.logInternal(
                "🔥 Swizzled: Handling universal link - \(userActivity.webpageURL ?? URL(fileURLWithPath: ""))"
            )
    ...
    extension NSObject {
        ...
        @objc func swizzled_scene(_ scene: UIScene, continue userActivity: NSUserActivity) {
            Logger.logInternal(
                "🔥 Swizzled method called: scene(_ scene: UIScene, continue userActivity: NSUserActivity)"
            )
        ...
        @objc func swizzle_scene_continue(
